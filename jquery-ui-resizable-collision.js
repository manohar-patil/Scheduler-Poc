...
(function($){
	...
	$.ui.plugin.add( "resizable", "restraint", {
		create: function(event,ui){         handleInit   .call( this, event, ui ); },
		start: function(event,ui){ if( ! $(this).data("ui-resizable").options.obstacle ) // if there are obstacles, we already handled both
															 {
																				handleStart  .call( this, event, ui );
															 }
														 } ,
		resize: function (event, ui) {
		    if (!$(this).data("ui-resizable").options.obstacle) // if there are obstacles, we already handled both
															 {
																 return handleCollide.call( this, event, ui );
															 }
														 } ,
		stop:  function(event,ui){ if( ! $(this).data("ui-resizable").options.obstacle ) // if there are obstacles, we already handled both
															 {
	...
	$.ui.plugin.add( "resizable", "multipleCollisionInteractions", {
		create: function(event,ui){         handleInit   .call( this, event, ui ); },
		start: function(event,ui){ if( ! $(this).data("ui-resizable").options.obstacle &&
																	 ! $(this).data("ui-resizable").options.restraint   )
															 {
																				handleStart  .call( this, event, ui );
															 }
														 } ,
		resize:  function(event,ui){ if( ! $(this).data("ui-resizable").options.obstacle &&
																	 ! $(this).data("ui-resizable").options.restraint   )
															 {
																 return handleCollide.call( this, event, ui );
															 }
														 } ,
		stop:  function(event,ui){ if( ! $(this).data("ui-resizable").options.obstacle &&
																	 ! $(this).data("ui-resizable").options.restraint   )
															 {
																				handleCollide.call( this, event, ui );
																				handleStop   .call( this, event, ui );
	...
	function handleStart(event,ui)
	{
		...
		$(this).data( "jquery-ui-resizable-collision-recent-position", ui.originalPosition );
	}
	...
	{
		// Note that $(this) is the resizable that's moving - it has a ui.position that moves acording to where
		...
		if( ilist.length <= 0 )
		{
			...
			$(this).data( "jquery-ui-resizable-collision-recent-position", ui.position );
			return;
		}
		...
		// Check if there's containment for the resizable:
		var $cont = $();
		if( widget.containment )
		{
			maxiter += 2; // the main resizable container times twice the number of containers (1)

			var cn = widget.containment;
			$cont = $("<div />").offset( { left: cn[0], top: cn[1] } )
													.width(  cn[2]-cn[0]+$(this).width( ) )  // because it had the resizable's size chopped out :-P
													.height( cn[3]-cn[1]+$(this).height() ); // because it had the resizable's size chopped out :-P
		...
		while( iter < maxiter )
		{
			...
			if( widget.containment )
			{
				...
				// NOTE: since resizable plugin has already applied containment, if we accidentally move it outside, it won't fix it for us
